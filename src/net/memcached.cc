...
void Memcached::upload_files(const vector<storage::PutRequest> & upload_requests, const function<void( const storage::PutRequest & )> & success_callback)
{
    ...
    for (size_t thread_index = 0; thread_index < thread_count; thread_index++) {
        if (thread_index < upload_requests.size()) {
            ...
            [&] (const size_t index)
            {
                log_debug("thread index [%d] begin construct MemcachedClient obj", thread_index);
                MemcachedClient client(this->_config);
                log_debug("thread index [%d] end construct MemcachedClient obj", thread_index);

                log_debug("thread index [%d] begin connect to memcached server", thread_index);
                if (client.Connect() < 0) {
                    log_error("thread index [%d] error connecting to memcached server", thread_index);
                    throw runtime_error("error connecting to memcached server");
                }
                log_debug("thread index [%d] end connect to memcached server", thread_index);
                ...
                for (size_t first_file_idx = index; first_file_idx < upload_requests.size(); first_file_idx += thread_count * batch_size) {
                    ...
                    for (size_t file_id = first_file_idx; file_id < min(upload_requests.size(), first_file_idx + thread_count * batch_size); file_id += thread_count ) {
                        ...
                        if (client.Set(object_key, contents) < 0) {
                            log_error("thread index [%d] failed to set key [%s] to memcached", thread_index, object_key.c_str());
                            throw runtime_error("failed to set key");
                        }
            ...
            [&] (const size_t index)
            {
                ...
                for (size_t first_file_idx = index; first_file_idx < download_requests.size(); first_file_idx += thread_count * batch_size) {
                    for (size_t file_id = first_file_idx; file_id < min(download_requests.size(), first_file_idx + thread_count * batch_size); file_id += thread_count) {
                        ...
                        if (client.Get(object_key, str_data) < 0) {
                            log_error("thread index [%d] failed to get key [%s] from memcached", thread_index, object_key.c_str());
                            throw runtime_error("failed to get key");
                        }
